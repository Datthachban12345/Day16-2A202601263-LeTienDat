...
class InjectionGuard(Middleware):
    ...
    def wrap_tool_call(self, ctx, call, name, args):
        ...
        if BLOCK_START not in (result.content or ""):
            return result

        # Cắt tất cả các đoạn độc
        content = result.content
        while BLOCK_START in content:
            start_idx = content.find(BLOCK_START)
            end_idx = content.find(BLOCK_END, start_idx)
            if end_idx == -1:
                # Không tìm thấy BLOCK_END, cắt từ BLOCK_START tới hết chuỗi
                content = content[:start_idx] + PLACEHOLDER
            else:
                # Cắt từ BLOCK_START tới hết BLOCK_END
    ...
    def after_agent(self, ctx, report):
        ...
        if INJECTION_CANARY in answer:
            report["answer"] = answer.replace(INJECTION_CANARY, "[đã gỡ]")
        return report
